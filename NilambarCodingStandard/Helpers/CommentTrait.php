...
trait CommentTrait {
	...
	protected function find_comment_tags( File $phpcsFile, int $commentStart ): array {
		...
		// Get the actual file line number for the comment start.
		$commentStartLine = $tokens[ $commentStart ]['line'];

		$parsedComment = $this->get_parsed_comment_details( $commentContent, $commentStartLine, $phpcsFile, $commentStart );
	...
	protected function get_parsed_comment_details( string $commentComment, int $commentStartLine = 0, File $phpcsFile = null, int $commentStart = 0 ): array {
		...
		foreach ( $lines as $lineNumber => $line ) {
			...
			} elseif ( strpos( $trimmedLine, '@' ) === 0 ) { // Check if line is a standalone tag (without *).
				$lineDetails['line_type'] = 'tag';
				$lineDetails['is_tag']    = true;
